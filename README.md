...
The final workflow step resolves that draft through the authenticated release list and its numeric
release ID because GitHub's `GET /releases/tags/{tag}` API returns 404 for draft releases.
...
GitHub Actions must be enabled, and repository or organization policy must allow this workflow to
run its configured actions. A repository-wide read-only `GITHUB_TOKEN` default is compatible with
this workflow: it keeps the default permission at `contents: read` and grants `contents: write`
only to the two release-publishing jobs. A more restrictive organization policy that prevents those
jobs from receiving write access must be relaxed for releases to publish.
