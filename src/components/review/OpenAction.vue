...
const disabledReason = computed(() => {
  if (!props.workItem.worktreePath) return 'This branch needs to be checked out before it can be opened';
...
async function openWith(application: OpenApplication) {
  if (!props.workItem.worktreePath || !application.available) return;
  error.value = null;
  try {
    await launch(application.id, props.project.id, props.workItem.worktreePath);
...
<template>
  <div ref="root" class="open-control">
    ...
    <div v-if="menuOpen" class="open-menu">
      <p v-if="!workItem.worktreePath" class="open-menu__notice">
      ...
      <button
        ...
        :key="application.id"
        type="button"
        :disabled="!workItem.worktreePath || !application.available"
